...
class diff_pairs(DeviceBase):
    ...
    def genDeviceLayout(self):
        ...
        metal_via_count = math.ceil(via_count/3)
                ...
                if min_bottom_step != 0:
                    ...
                    if i % 2 != 0 :
                        ...
                        if via_width == 0:
                            via_width = width
                            via_height = height
                        gat_box = Box(via_step_x, via_y_offset, via_step_x + l_com, via_y_offset - via_height)
                        dbCreateRect(self, poly_layer, gat_box)
                        self.genVia( via_count ,1,via_x_offset, via_2_y_offset)
                        gat_box = Box(via_step_x, via_2_y_offset, via_step_x + l_com, via_2_y_offset - via_height)
                        dbCreateRect(self, poly_layer, gat_box)
                        # link sources together
                        met1_box = Box(device_step_x - x_offset - cont_Activ_overRec + min_origin_distance,  device_step_y*(i), device_step_x + cont_Activ_overRec, - device_step_y*i - y_offset + min_bottom_step)
                        ...
                        if j == 0 and via_width/3 >= min_metal_width and self.cg:
                            ...
                            met2_box = Box(via_x_offset + 2*via_width/3, via_2_y_offset, via_step_x_start + device_step_x*(j+1) + l_com/3, via_2_y_offset - via_height);
                            dbCreateRect(self, metal2_layer, met2_box)
                            met3_box = Box(via_x_offset, via_y_offset, via_step_x_start + device_step_x*(j+1) + l_com/3, via_y_offset - via_height);
                            dbCreateRect(self, metal3_layer, met3_box)
        ...
        x_offset = -contact_distance + min_origin_distance;
        ## generate dummies
        self.dd = self.dd
        ...
        for i in range(self.nr):
            for j in range(self.nd):
                ...
                if i % 2 != 0 :
                    ...
                    gat_box = Box(via_step_x, via_step_y, via_step_x + l_com, via_step_y - via_height)
                    dbCreateRect(self, poly_layer, gat_box)
                    via_2_step_y = - y_offset + 0.41 + min_bottom_step + device_step_y*(i-1)
                    self.genVia( via_count ,1,via_step_x,  via_2_step_y)
                    gat_box = Box(via_step_x, via_2_step_y, via_step_x + l_com, via_2_step_y - via_height)
                    dbCreateRect(self, poly_layer, gat_box)
